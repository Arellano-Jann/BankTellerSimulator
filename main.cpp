...
int main(){
		...
		std::cout << "60.";
	// Customer customer;
	ArrayQueue<Customer> fileQueue; // initial queue
	// ArrayQueue<Customer> bankQueue; // bank line
	// PriorityQueue<EventTracker> EventQueue;
	// bool isTellerAvailable = true;
	// int currentTime = 0;
	std::cout << std::boolalpha << fileQueue.isEmpty();
	Customer custom(5,6);
	std::cout << custom.getArrivalTime() << custom.getWaitingTime();
	std::cout << std::boolalpha << fileQueue.enqueue(custom);
	std::cout << fileQueue.peekFront().getArrivalTime();
	// std::cout << fileQueue.
	// std::cout << fileQueue.

		std::cout << "60.";
		std::ifstream files("file.txt");
	// std::ifstream files("file.txt", std::ios::in);
		std::cout << "60.";
	if (files){
		std::cout << "60.";
		int arrivalTime, waitingTime;
		while (files >> arrivalTime >> waitingTime){
			// Customer customr(arrivalTime, waitingTime); // if this doesn't work use getters and setters
